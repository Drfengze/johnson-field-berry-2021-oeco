...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Python translation of run_forward_example2.m."
    )
    parser.add_argument(
        "--pathway",
...
def _build_example2_data(n):
    return SimpleNamespace(
        Cin=np.linspace(1, 1000, n),
...
def main():
    ...
    outputname = f"Example2-CO2-{pathway_option}"
    outputdir = resultsdir / f"{outputname}{args.output_suffix}"
    outputdir.mkdir(parents=True, exist_ok=True)

    n = 100
    data = _build_example2_data(n)
