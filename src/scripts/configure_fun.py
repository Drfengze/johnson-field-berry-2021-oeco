...
def configure_fun(data):
    ...
    Q = np.asarray(_field(data, "Qin"), dtype=float) / 1e6
    Q_u = "mol PAR m-2 s-1"
    Q_d = "PPFD"

    T = np.asarray(_field(data, "Tin"), dtype=float)
    T_u = "degrees C"
    T_d = "Leaf temperature"

    P = np.asarray(_field(data, "Pin"), dtype=float)
    P_u = "bar"
    P_d = "Total pressure"

    C_m = np.asarray(_field(data, "Cin"), dtype=float) / 1e6
    C_m_u = "bar"
    C_m_d = "Partial pressure of CO2"

    O_m = np.asarray(_field(data, "Oin"), dtype=float)
    O_m_u = "bar"
