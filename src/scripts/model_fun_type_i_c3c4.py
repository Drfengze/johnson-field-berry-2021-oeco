...
def model_fun_type_i_c3c4(v):
    ...
    if not _strcmp(pathway_option, "Type-I-C3-C4"):
        raise ValueError(
            f"model_fun_type_i_c3c4 only supports pathway_option='Type-I-C3-C4', got {pathway_option!r}"
        )

    if not _all_true(V_p_max_m == 0):
        raise ValueError("model_fun_type_i_c3c4 expects V_p_max to be zero for the Type-I-C3-C4 pathway.")
    ...
    if np.mean(np.asarray(V_c_max_s, dtype=float)) > 0 and np.mean(np.asarray(V_p_max_m, dtype=float)) == 0:
        J_PSI_s_jj = PPFD * V_q_max_s / (PPFD + V_q_max_s / (a_PSI_s * (k_P1 / (k_P1 + k_D + k_F))))
        A_net_s_jj = solve_C3C4_jj(CO2_m, J_PSII_m_j, J_PSI_s_jj, O2_m, Pressure, R_d_s, Specificity, g_bs_CO2, g_bs_O2, ATP_e_ratio_cyclic, ATP_e_ratio_linear)
        CO2_s_jj = CO2_m + (
            (J_PSII_m_j * O2_m / (2 * Specificity * CO2_m) / (4 * (1 + O2_m / (Specificity * CO2_m)))) - A_net_s_jj
        ) * Pressure / g_bs_CO2
        O2_s_jj = O2_m + A_net_s_jj * Pressure / g_bs_O2
        A_gross_s_jj = A_net_s_jj + R_d_s
        J_PSII_s_jj = A_gross_s_jj * 4 * (1 + O2_s_jj / (Specificity * CO2_s_jj)) / (1 - O2_s_jj / (2 * Specificity * CO2_s_jj))
        V_c_s_jj = J_PSII_s_jj / (4 * (1 + O2_s_jj / (Specificity * CO2_s_jj)))
        V_o_s_jj = V_c_s_jj * O2_s_jj / (Specificity * CO2_s_jj)

        A_net_s_jc = solve_C3C4_jc(CO2_m, J_PSII_m_j, K_m_CO2, K_m_O2, O2_m, Pressure, R_d_s, Specificity, V_c_max_s, g_bs_CO2, g_bs_O2)
        CO2_s_jc = CO2_m + (
            (J_PSII_m_j * O2_m / (2 * Specificity * CO2_m) / (4 * (1 + O2_m / (Specificity * CO2_m)))) - A_net_s_jc
        ) * Pressure / g_bs_CO2
        O2_s_jc = O2_m + A_net_s_jc * Pressure / g_bs_O2
        A_gross_s_jc = A_net_s_jc + R_d_s
        J_PSII_s_jc = A_gross_s_jc * 4 * (1 + O2_s_jc / (Specificity * CO2_s_jc)) / (1 - O2_s_jc / (2 * Specificity * CO2_s_jc))
        J_PSI_s_jc = J_PSII_s_jc * (
            1
            - (ATP_e_ratio_linear / ATP_e_ratio_cyclic)
            + (3 + 7 * O2_s_jc / (2 * Specificity * CO2_s_jc)) / ((4 + 4 * O2_s_jc / (Specificity * CO2_s_jc)) * ATP_e_ratio_cyclic)
        )
        V_c_s_jc = J_PSII_s_jc / (4 * (1 + O2_s_jc / (Specificity * CO2_s_jc)))
        V_o_s_jc = V_c_s_jc * O2_s_jc / (Specificity * CO2_s_jc)

        J_PSI_s_cj = J_PSI_s_jj
        A_net_s_cj = solve_C3C4_cj(CO2_m, J_PSI_s_cj, K_m_CO2, K_m_O2, O2_m, Pressure, R_d_s, Specificity, V_c_max_m, g_bs_CO2, g_bs_O2, ATP_e_ratio_cyclic, ATP_e_ratio_linear)
        CO2_s_cj = CO2_m + (
            (V_c_max_m * O2_m / (2 * Specificity) / (CO2_m + K_m_CO2 * (1 + O2_m / K_m_O2))) - A_net_s_cj
        ) * Pressure / g_bs_CO2
        O2_s_cj = O2_m + A_net_s_cj * Pressure / g_bs_O2
        A_gross_s_cj = A_net_s_cj + R_d_s
        J_PSII_s_cj = A_gross_s_cj * 4 * (1 + O2_s_cj / (Specificity * CO2_s_cj)) / (1 - O2_s_cj / (2 * Specificity * CO2_s_cj))
        V_c_s_cj = J_PSII_s_cj / (4 * (1 + O2_s_cj / (Specificity * CO2_s_cj)))
        V_o_s_cj = V_c_s_cj * O2_s_cj / (Specificity * CO2_s_cj)

        A_net_s_cc = solve_C3C4_cc(CO2_m, K_m_CO2, K_m_O2, O2_m, Pressure, R_d_s, Specificity, V_c_max_m, V_c_max_s, g_bs_CO2, g_bs_O2)
        CO2_s_cc = CO2_m + (
            (V_c_max_m * O2_m / (2 * Specificity) / (CO2_m + K_m_CO2 * (1 + O2_m / K_m_O2))) - A_net_s_cc
        ) * Pressure / g_bs_CO2
        O2_s_cc = O2_m + A_net_s_cc * Pressure / g_bs_O2
        A_gross_s_cc = A_net_s_cc + R_d_s
        J_PSII_s_cc = A_gross_s_cc * 4 * (1 + O2_s_cc / (Specificity * CO2_s_cc)) / (1 - O2_s_cc / (2 * Specificity * CO2_s_cc))
        J_PSI_s_cc = J_PSII_s_cc * (
            1
            - (ATP_e_ratio_linear / ATP_e_ratio_cyclic)
            + (3 + 7 * O2_s_cc / (2 * Specificity * CO2_s_cc)) / ((4 + 4 * O2_s_cc / (Specificity * CO2_s_cc)) * ATP_e_ratio_cyclic)
        )
        V_c_s_cc = J_PSII_s_cc / (4 * (1 + O2_s_cc / (Specificity * CO2_s_cc)))
        V_o_s_cc = V_c_s_cc * O2_s_cc / (Specificity * CO2_s_cc)
